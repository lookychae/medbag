...
export default function DetailScreen({ selected, memos, saveMemos, memoEditing, setMemoEditing, setScreen, onEdit }) {
  const currentMemo = memos[selected.id] ?? selected.memo ?? "";
  return (
    <div style={{paddingBottom:40}}>
      {/* Header */}
      <div style={{background:selected.accent,padding:`${SAFE_TOP(14)} 22px 16px`,position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",top:-30,right:-30,width:160,height:160,borderRadius:"50%",background:"rgba(255,255,255,0.08)"}}/>
        <button onClick={()=>setScreen("home")} style={{position:"absolute",top:SAFE_TOP(4),left:16,background:"rgba(255,255,255,0.2)",border:"none",borderRadius:"50%",width:40,height:40,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0,zIndex:10}}>
          <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
            <polyline points="15 18 9 12 15 6"/>
          </svg>
        </button>
        {onEdit && (
          <button onClick={()=>onEdit(selected)} style={{position:"absolute",top:SAFE_TOP(12),right:16,background:"rgba(255,255,255,0.2)",border:"none",borderRadius:10,padding:"6px 14px",color:"white",fontSize:13,fontWeight:600,cursor:"pointer",zIndex:10}}>수정</button>
        )}
      ...
      <div style={{padding:"16px 18px",display:"flex",flexDirection:"column",gap:10}}>
        ...
        {selected.medicines.map((m,i) => (
          <div key={i} style={{background:"white",borderRadius:14,padding:"14px 16px",boxShadow:"0 2px 8px rgba(0,0,0,0.06)",borderLeft:`3px solid ${catColor(m)}`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
              ...
              <span style={{fontSize:12,background:catColor(m)+"20",color:catColor(m),borderRadius:6,padding:"2px 8px",fontWeight:700,flexShrink:0}}>{m.category}</span>
            </div>
            <div style={{display:"flex",gap:8}}>
              {[
                ...
                <div key={j} style={{flex:1,textAlign:"center",background:info.hi?catColor(m)+"10":"#F9F9F9",borderRadius:10,padding:"10px 4px",border:info.hi?`1px solid ${catColor(m)}25`:"1px solid #F0F0F0",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:4}}>
                  {info.hi ? <MedBadge dosage={m.dosage} color={catColor(m)}/> : <div style={{fontSize:15,fontWeight:600,color:"#1C1C1E",lineHeight:1.3}}>{info.value}</div>}
        ...
        <div style={{background:"white",borderRadius:14,padding:"14px 16px",boxShadow:"0 2px 8px rgba(0,0,0,0.06)",marginTop:2}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            ...
            {!memoEditing ? (
              ...
              <button onClick={()=>setMemoEditing(false)} style={{background:COLORS.navy,border:"none",borderRadius:8,padding:"6px 14px",fontSize:12,fontWeight:700,color:"white",cursor:"pointer"}}>저장</button>
            )}
          </div>
          {memoEditing ? (
            <textarea autoFocus placeholder="복약 상황, 부작용, 기타 참고 메모를 자유롭게 입력해주세요"
              value={currentMemo} onChange={e=>saveMemos({...memos,[selected.id]:e.target.value})}
            ...
            <div style={{fontSize:15,color:"#3C3C3C",lineHeight:1.8,padding:"12px 14px",background:"#FAFAFA",borderRadius:10,whiteSpace:"pre-wrap"}}>{currentMemo}</div>
          ) : (
