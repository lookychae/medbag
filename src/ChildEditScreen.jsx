// 아이 정보 편집 폼 (프로필·측정치·성장 로그·메모).
import { COLORS, GRADIENTS, SAFE_TOP } from "./theme";

function Field({ value, onChange, type="text", placeholder="", opts=null }) {
  const base = {flex:1,border:"none",borderBottom:`1.5px solid ${COLORS.border}`,padding:"6px 0",fontSize:14,fontWeight:600,outline:"none",color:COLORS.textPrimary,background:"transparent"};
  if (opts) return (
...
export default function ChildEditScreen({ profileDraft, setProfileDraft, saveChildProfile, setScreen }) {
  if (!profileDraft) return null;

  const set = (key, val) => setProfileDraft(p => ({...p, [key]:val}));

  // 편집 중에는 문자열 그대로 보관 (빈 칸 허용). 숫자 변환은 handleSave에서.
  const updateLog = (logKey, idx, field, val) => {
    const logs = [...(profileDraft[logKey]||[])];
    logs[idx] = {...logs[idx], [field]: field==="value" ? val : val+"-01"};
    setProfileDraft(p => ({...p, [logKey]:logs}));
  };

  // 현재 키/몸무게 입력 — 빈 칸일 때는 로그 변경 없이 빈 채로 유지.
  const updateMeasurement = (key, logKey, val) => {
    if (val === "") {
      setProfileDraft(p => ({...p, [key]: ""}));
      return;
    }
  ...
  const addLog = (logKey) => setProfileDraft(p => ({...p, [logKey]:[...(p[logKey]||[]), {date:new Date().toISOString().slice(0,7)+"-01",value:""}]}));
  const removeLog = (logKey, idx) => setProfileDraft(p => ({...p, [logKey]:p[logKey].filter((_,j)=>j!==idx)}));

  // 저장 직전에 빈 칸/NaN을 0으로 정규화.
  const handleSave = () => {
    const toNum = v => { const n = parseFloat(v); return isNaN(n) ? 0 : n; };
    const normalized = {
      ...profileDraft,
      height: toNum(profileDraft.height),
      weight: toNum(profileDraft.weight),
      heightLog: (profileDraft.heightLog||[]).map(l => ({...l, value: toNum(l.value)})),
      weightLog: (profileDraft.weightLog||[]).map(l => ({...l, value: toNum(l.value)})),
    };
    saveChildProfile(normalized);
    setScreen("child");
  };
  ...
  return (
    <div style={{background:"#F2F2F7",minHeight:"100vh",paddingBottom:40}}>
      {/* Header */}
      <div style={{background:GRADIENTS.header,padding:`${SAFE_TOP(12)} 16px 14px`,position:"relative"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <button onClick={handleSave} style={{background:COLORS.navy,border:"1.5px solid rgba(255,255,255,0.3)",borderRadius:10,padding:"6px 14px",color:"white",fontSize:13,fontWeight:700,cursor:"pointer",flexShrink:0}}>저장</button>
        </div>
      </div>
      ...
      <div style={{padding:"16px 18px",display:"flex",flexDirection:"column",gap:12}}>
        ...
        <div style={{background:"white",borderRadius:14,padding:"20px 16px",boxShadow:"0 2px 8px rgba(0,0,0,0.05)",display:"flex",flexDirection:"column",alignItems:"center",gap:10}}>
          <div style={{width:72,height:72,borderRadius:22,background:GRADIENTS.primary,display:"flex",alignItems:"center",justifyContent:"center",fontSize:36}}>
            {profileDraft.gender==="여"?"👧":"🧒"}
          </div>
          <div style={{display:"flex",gap:10}}>
            {["남","여"].map(g => (
              <button key={g} onClick={()=>set("gender",g)} style={{padding:"8px 28px",borderRadius:10,cursor:"pointer",fontSize:14,fontWeight:700,border:profileDraft.gender===g?"none":`1.5px solid ${COLORS.border}`,background:profileDraft.gender===g?COLORS.navy:"white",color:profileDraft.gender===g?"white":COLORS.textSecondary}}>
                {g==="남"?"🧒 남아":"👧 여아"}
              </button>
            ))}
          </div>
        ...
        {/* Growth logs */}
        <div style={{background:"white",borderRadius:14,padding:"16px",boxShadow:"0 2px 8px rgba(0,0,0,0.05)"}}>
          <div style={{fontSize:13,fontWeight:700,color:"#1C1C1E",marginBottom:16}}>📊 성장 기록</div>
          {[
        ...
        <button onClick={handleSave} style={{background:COLORS.navy,border:"none",borderRadius:14,padding:"16px",color:"white",fontSize:15,fontWeight:700,cursor:"pointer",marginTop:4,boxShadow:"0 4px 14px rgba(0,0,0,0.15)"}}>저장하기</button>
      </div>
    </div>
  );
}
