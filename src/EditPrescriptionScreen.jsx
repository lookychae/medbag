...
export default function EditPrescriptionScreen({ rx, onCancel, onSave }) {
  ...
  useEffect(() => {
    ...
    // iOS PWA에서 안전영역 뒤로 body 배경이 비치는 걸 막기 위해 강제로 다크 네이비로.
  ...
  const handleSave = () => {
    ...
    onSave({ ...form, medicines: form.medicines.filter(m => m.name).map(m => ({...m, dosage:`${m.dosageAmt}${m.dosageUnit}`})) });
  };
    ...
    <div style={{width:"100%",maxWidth:500,height:"100%",background:COLORS.navy,display:"flex",flexDirection:"column",overflowX:"hidden"}}>
      ...
      <div style={{flexShrink:0,padding:`${SAFE_TOP(18)} 18px 16px`,position:"relative"}}>
        ...
        <div style={{textAlign:"center",marginTop:8}}>
          ...
          <div style={{color:"white",fontSize:17,fontWeight:700}}>처방전 수정</div>
          <div style={{color:"rgba(255,255,255,0.4)",fontSize:12,marginTop:4}}>내용을 수정하고 저장하세요</div>
        </div>
      </div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 18px"}}>
        ...
        <div style={{background:"rgba(255,255,255,0.05)",borderRadius:16,padding:16,marginBottom:12,border:"1px solid rgba(255,255,255,0.08)"}}>
          ...
          {[
            {label:"담당의사",key:"doctor",ph:"이름만 입력해주세요"},
